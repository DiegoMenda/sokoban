...
public class SokobanLogic {
	...
	public void undoMove() {
		...
		if (lastMove.isBoxMove()) {
			...
			if(world.getLevel().getImmovableEntities(lastMove.getNewBoxX(), lastMove.getNewBoxY()) instanceof Goal)  {
	...
	private void updateBoxState() {
		for(int y = 0; y < world.getLevel().getRow(); y++   ) {
			for(int x = 0; x < world.getLevel().getCol(); x++) {
				...
				if(world.getLevel().getImmovableEntities()[y][x] instanceof Goal && 
						world.getLevel().getMobileEntities()[y][x] instanceof Box) {
					((Goal)world.getLevel().getImmovableEntities()[y][x]).setGoalArchieved(true);
					((Box)world.getLevel().getMobileEntities()[y][x]).setBoxOnGoalTexture();
					logger.info("We reached the goal.");
				}
				else if(world.getLevel().getImmovableEntities()[y][x] instanceof Goal) {
					((Goal)world.getLevel().getImmovableEntities()[y][x]).setGoalArchieved(false);
				}
				else if(world.getLevel().getMobileEntities()[y][x] instanceof Box) {
					((Box)world.getLevel().getMobileEntities()[y][x]).setNormalBoxTexture();
				}
			}
		}
	}
